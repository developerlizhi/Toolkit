...
const config = {"filePath": "xxxx", "git_Address": "http://xxx","origin":"origin"};
...
async function start(filePath) {
    ...
    for (let i = 0; i < file_info.length; i++) {
        ...
        if(getOldAddress!="err"){
            ...
            await exelCmd(`cd ${filePath}/${file_info[i]}&&git remote rm ${config.origin}&&git remote add ${config.origin} ${config.git_Address}${file_info[i]}.git`,'F');
